Param (
     ...
     [int]
     $AMOSRequiredVersion = 26,
 
     # AMOS Key
     [Parameter(Mandatory=$false)]
     [string]
     $AMOSKey = "EnterKeyHere",
 
     # AMOS MSI Code
     [Parameter(Mandatory=$false)]
     [string]
     $AMOSMSICode  = "{C31442F6-4B76-4022-9822-7F60F3EA70D8}",
 
     # AMOS License updater exe
     [Parameter(Mandatory=$false)]
     [string]
     $AMOSLicenseUpdaterPath
)
...
Function Invoke-LicenseUpdate {
    ...
    try {
        ...
        Start-process -FilePath $appinfo.LicenseUpdaterPath -argumentlist $AMOSKey -redirectstandardoutput $licenselogfile
    }
    catch {
...
$logfile = "$($logdir)\script_Update-AMOSLicense.log"


if ([STRING]::IsNullOrEmpty($AMOSLicenseUpdaterPath)) {
    $AMOSLicenseUpdaterPath = "$(${env:ProgramFiles(x86)})\IBM\SPSS\Amos\$AMOSRequiredVersion\licenseactivator.exe"
}

$ApplicationInfo = @{
    Product             = "AMOS"
    RequiredVersion     = $AMOSRequiredVersion
    Key                 = $AMOSKey
    MSICode             = $AMOSMSICode
    LicenseUpdaterPath  = $AMOSLicenseUpdaterPath
    UpdateResult        = ""
}


Write-Logfile -path $logfile -loglevel information -message "Checking for AMOS version $($ApplicationInfo.Requiredversion)."
